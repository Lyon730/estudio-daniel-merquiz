...
async function saveServicios() {
  ...
  if (database) {
    try {
      await database.ref('servicios').set(serviciosData);
    } catch (e) {
    ...
    servicio.grupos.forEach((grupo, gIdx) => {
      ...
      const opcionesHTML = grupo.opciones.map((op, oIdx) => `
        <div class="servicios-admin-opcion">
          ...
          <input type="number" class="sa-precio-input" value="${op.precio}" min="0" step="100"
            onchange="updateServicioPrecio(${sIdx}, ${gIdx}, ${oIdx}, this.value)" />
