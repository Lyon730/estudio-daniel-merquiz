...
async function diagnosticarFirebase() {
  ...
  try {
    ...
    if (e.code === 'PERMISSION_DENIED') {
      lines.push('');
      lines.push('🔧 <strong>Solución:</strong> Ve a <a href="https://console.firebase.google.com/project/estudio-daniel-merquiz/database/estudio-daniel-merquiz-default-rtdb/rules" target="_blank" style="color:#d4af37;">Firebase Console → Realtime Database → Reglas</a> y reemplaza las reglas por:');
      lines.push('<pre style="background:#1c1c1c;padding:10px;border-radius:6px;margin-top:6px;font-size:0.82rem;overflow:auto;">{\n  "rules": {\n    ".read": true,\n    ".write": true\n  }\n}</pre>');
      lines.push('<small style="color:#f08080;">Nota: estas reglas permiten acceso público. Suficiente para este proyecto ya que el panel admin requiere contraseña.</small>');
    }
  }
